...
#######################################################
    ...
    @staticmethod
    def rectified_original_dataset_augment_single_size_patches(in_path=settings.ORIGINAL_TRAINING_PATH, out_path=settings.ORIGINAL_VIOLA_PATCHES_AUGM):
        '''
        No division between different roof sizes: if a roof has a size that is off, we resize it
        Make them lie down, save patches to folder
        Augment patches, save them to augmented folder
        ...
        for img_id, img_name in enumerate(img_names_list):
			...
			polygons = DataLoader().get_roofs_rectified_dataset('../data/bounding_inhabited/'+img_name[:-3]+'xml')
			for i, polygon in enumerate(polygons):
				print polygon
				pdb.set_trace()
				warped = four_point_transform(img, np.array(polygon, dtype = "float32"))
	
            roofs = roof_loader.get_roofs_rectified_dataset(xml_path, img_name)
            metal_log = list()
            thatch_log = list()
            try:        
                img = cv2.imread(img_path, flags=cv2.IMREAD_COLOR)
