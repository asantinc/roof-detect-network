...
class Pipeline(object):
	...
	def __init__(self, test_files=None):
		...
		if test_files is None:
			self.test_fnames = DataLoader.get_img_names_from_path(
		...
		self.viola = ViolaDetector(
		    detector_paths=self.detector_paths, output_folder=self.out_path, save_imgs=True)
	...
	def process_viola(self, img_name, image, verbose=False):
		...
		'''
		# returns list with as many lists of detections as the detectors we have
		# passed
		detections = self.viola.detect_roofs(
		    img_name=img_name, img_path=settings.TEST_PATH)
		if verbose:
			self.viola.mark_detections_on_img(self, img=image, img_name=img_name)

		# get the mask and the contours for the detections
		detection_mask = self.viola.get_patch_mask(
		    img_name=img_name, rows=rows, cols=cols)
		self.all_contours[img_name] = self.viola.get_detection_contours(
		    self, detection_mask, img_name)
	...
	def sliding_convolution(self, img, contours, step_size):
		...
		for cont in contours:
	        ...
	        vert_patches = ((h - settings.PATCH_H) // self.step_size) + 1
	...
	def get_horizontal_patches(self, patch=None, img=None, roof=None, y_pos=-1):
	    ...
	    hor_patches = ((w - settings.PATCH_W) // self.step_size) + 1

	    for horizontal in range(hor_patches):
	        x_pos = roof.xmin + (horizontal * self.step_size)
	        candidate = img[
	            x_pos:x_pos + settings.PATCH_W, y_pos:y_pos + settings.PATCH_H]
